...
import { SEARCH_REPOS, ADD_STAR, REMOVE_STAR } from '~/graphql/documents.js'
...
const { mutate: starRepo } = useMutation(ADD_STAR, () => ({
  variables: {
...
const { mutate: unstarRepo } = useMutation(REMOVE_STAR,
  () => ({
    variables: {
      repositoryId: selectedId,
    },
...
const handleStarBtnClick = (evt) => {
  ...
  if (viewerHasStarred) unstarRepo()
  else starRepo()
}
</script>
...
<template>
  ...
  >
    ...
    <div class="min-w-0 relative flex-auto sm:pr-20 lg:pr-0 xl:pr-20">
      ...
      <p v-if="repository.node.description" class="mb-2 w-10/12 truncate">
      ...
      <dl class="flex flex-wrap text-sm font-medium whitespace-pre mb-2">
        ...
        >
          <dt class="flex items-center mb-2">
            <span
              class="uppercase tracking-widest h5 block mr-1 font-semibold"
            >Stargazers</span>
            <button
              class="
                relative
